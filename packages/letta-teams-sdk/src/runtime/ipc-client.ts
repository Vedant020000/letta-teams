...
export function startDaemonInBackground(): number | null {
  ...
  // Prefer the currently running CLI entrypoint when available.
  // This keeps daemon bootstrap working after the SDK/CLI package split.
  const daemonPath = process.env.LETTA_TEAMS_CLI_ENTRY || process.argv[1];
  if (!daemonPath) {
    throw new Error("Unable to resolve letta-teams CLI entrypoint for daemon startup");
  }
